...
def show_timer():
    for i in range(timeout_duration):
        if site_connected:
            break
        sys.stdout.write(f"\râ³ [Admin ID: {ADMIN_ID}] Waiting for site response... {i + 1}/{timeout_duration} sec")
        sys.stdout.flush()
        time.sleep(1)
...
def check_site_connection():
    ...
    for attempt in range(1, max_retries + 1):
        site_connected = False
        print(f"\nðŸ”„ [Admin ID: {ADMIN_ID}] Attempt {attempt} to connect to Laravel site...")
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"\nâœ… [Admin ID: {ADMIN_ID}] Successfully connected to Laravel site.")
                return
            else:
                try:
                ...
                print(f"\nâŒ [Admin ID: {ADMIN_ID}] Status: {response.status_code}, Response: {error_detail}")

        except requests.exceptions.RequestException as e:
            site_connected = False
            timer_thread.join()
            print(f"\nâŒ [Admin ID: {ADMIN_ID}] Error: {str(e)}")

        if attempt < max_retries:
            wait_time = retry_delay * attempt
            print(f"ðŸ” [Admin ID: {ADMIN_ID}] Retrying in {wait_time} seconds...\n")
            time.sleep(wait_time)
        else:
            print(f"ðŸš¨ [Admin ID: {ADMIN_ID}] All retry attempts failed. Bypassing this connection attempt.")
            return
...
async def start(client, message: Message):
    ...
    welcome_message = (
        f"ðŸ‘‹ Há´‡ÊŸÊŸá´, {user_name}!\n\n"
        f"ðŸŽ¥ I'á´ Êá´á´œÊ€ á´˜á´‡Ê€sá´É´á´€ÊŸ Má´á´ Éªá´‡ & TV SÊœá´á´¡ á´€ssÉªsá´›á´€É´á´›. "
        f"Já´œsá´› á´›Êá´˜á´‡ á´›Êœá´‡ É´á´€á´á´‡ á´Ò“ á´€É´Ê á´á´á´ Éªá´‡ á´Ê€ sá´‡Ê€Éªá´‡s, "
        f"á´€É´á´… Iâ€™ÊŸÊŸ Ò“á´‡á´›á´„Êœ á´…á´‡á´›á´€ÉªÊŸs ÉªÉ´sá´›á´€É´á´›ÊŸÊ.\n\n"
        f"ðŸš€ Lá´‡á´›'s É¢á´‡á´› sá´›á´€Ê€á´›á´‡á´…!\n\n"
        f"ðŸ™Œ **CÊ€á´‡á´…Éªá´›s**:\n"
        f"ðŸ‘¨â€ðŸ’» **Dá´‡á´ á´‡ÊŸá´á´˜á´‡Ê€**: [Abdul khalik](https://t.me/Attitude2688)\n" 
        f"ðŸ‘‘ **Oá´¡É´á´‡Ê€**: [Abdul Khalik](https://t.me/Attitude2688)"
    )
    
    image_url = "https://telegra.ph/file/5d32303d074c709406576.jpg"
    buttons = [
        [InlineKeyboardButton("Aá´…á´… á´á´‡ ÉªÉ´ É¢Ê€á´á´œá´˜", url=f"https://t.me/{BOT_NAME}?startgroup=true")],
        [
            InlineKeyboardButton("API Sá´›á´€á´›á´œs", callback_data="api_status"),
            InlineKeyboardButton("DB Sá´›á´€á´›á´œs", callback_data="db_status")
        ],
        [InlineKeyboardButton("Bá´á´› Dá´‡á´ á´‡ÊŸá´á´˜á´‡Ê€", url="https://t.me/Attitude2688")]
    ]
...
async def broadcast(client: Client, message: Message):
    ...
    if not message.photo and not broadcast_message:
        await message.reply("âš ï¸ Usage: /broadcast <message> or send a photo with an optional caption.")
        return
    ...
    try:
        ...
        await message.reply("âŒ Error accessing user database.")
        return

    if total_users == 0:
        await message.reply("ðŸ˜• No users found to broadcast to.")
        return

    # Initialize counters
    success_count = 0
    failed_count = 0
    loading_msg = await message.reply(f"ðŸ“¢ Broadcasting to {total_users} users...")
    ...
    await loading_msg.edit(
        ...
        f"âŒ Failed to send to: {failed_count} users"
    )
...
async def user_count(client: Client, message: Message):
    ...
    try:
        total_users = users.count_documents({})
        await message.reply(f"ðŸ“Š Total users in database: {total_users}")
    except Exception as e:
...
async def handle_callback(client, callback_query):
    ...
    if data == "api_status":
        if user_id != ADMIN_ID:
            await callback_query.answer("ðŸš« You are not authorized to check API status.", show_alert=True)
            return
        check_site_connection()
        status = "âœ… Connected" if site_connected else "âŒ Not Connected"
        await callback_query.message.reply(f"ðŸ” API connection status: {status}")
        return

    if data == "db_status":
        if user_id != ADMIN_ID:
            await callback_query.answer("ðŸš« You are not authorized to check DB status.", show_alert=True)
            return
        try:
            mongo.server_info()
            await callback_query.message.reply("âœ… Database is connected.")
        except Exception as e:
            await callback_query.message.reply(f"âŒ Database error: {str(e)}")
        return
...
async def api_command(client: Client, message: Message):
    ...
    if user_id != ADMIN_ID:
        ...
        await client.send_message(
            ADMIN_ID,
            f"âš ï¸ Unauthorized attempt to use /api command by {user_name} (ID: {user_id})."
        )
        return

    check_site_connection()
    status = "âœ… Connected" if site_connected else "âŒ Not Connected"
    await message.reply(f"ðŸ” Site connection status: {status}")
...
async def search_movie_or_tv(client, message: Message):
    if not site_connected:
        await message.reply("ðŸš« The bot is currently not connected to the site. Please try again later.")
        return
    ...
    await client.send_message(ADMIN_ID, f"ðŸ§ User `{username}` searched for: `{query}`")
    ...
    if not search_query:
        await loading_msg.edit("âš ï¸ Please provide a valid movie or TV show name.")
        return
    ...
    try:
        ...
        await loading_msg.edit(f"âš ï¸ TMDB API error: {e}. Please try again later.")
        return
    except Exception as e:
        logging.error(f"TMDb search failed for query '{search_query}': {e}")
        await loading_msg.edit("âš ï¸ Error while searching. Please try again later.")
        return

    if not results:
        await loading_msg.edit("ðŸ˜• No matching results found.")
        return
    ...
    if not filtered_results and results and search_year:
        await loading_msg.edit(
            f"âš ï¸ No results found for '{search_query}' in {search_year}. Showing closest matches instead:"
        )
        filtered_results = results
        result_types = ["movie" if r in movie_results else "tv" for r in results]
...
async def send_result(client, chat_id, user_id, index, loading_msg):
    ...
    for i in range(5):
        ...
        try:
            ...
            await client.send_message(chat_id, f"âš ï¸ TMDB API error: {e}. Skipping this result.")
            continue
        except Exception as e:
            logging.error(f"Error fetching details for ID {res_id}: {e}")
            await client.send_message(chat_id, "âš ï¸ Error fetching details. Skipping this result.")
            continue
    ...
    nav_buttons = []
    if index > 0:
        nav_buttons.append(InlineKeyboardButton("â¬…ï¸ Previous", callback_data="prev"))
    if index + 5 < len(result_ids):
        nav_buttons.append(InlineKeyboardButton("Next âž¡ï¸", callback_data="next"))
    if nav_buttons:
    ...
    try:
        ...
        logging.error(f"TMDb API error while fetching details for ID {res_id}: {e}")
        await loading_msg.edit(f"âš ï¸ TMDB API error: {e}. Cannot display this result.")
        return
    except Exception as e:
        logging.error(f"Error fetching details for ID {res_id}: {e}")
        await loading_msg.edit("âš ï¸ Error fetching details. Cannot display this result.")
        return
